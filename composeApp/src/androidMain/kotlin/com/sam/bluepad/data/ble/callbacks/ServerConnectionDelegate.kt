...
class ServerConnectionDelegate(
	...
	private suspend fun markSyncSessionPacketEnded(
		...
		return runCatching {
			...
			val result = processedResult.getOrElse { err -> return Result.failure(err) }

			// handle the result
			when (result) {
				is SyncDataPayload.ContentIdsQuery if (result.ids.isEmpty()) -> {
					// data is same no need to sync data
					val session = BLESyncSession.SyncSessionFailed(reason = BLESyncFailedReason.CONTENT_SAME, false)
				...
				is SyncDataPayload.SuccessAndNoAction -> {
					// HURRAY EVERYTHING WENT WELL
					val success = BLESyncSession.SyncSessionSuccessful
					val bytes = protoBuf.encodeToByteArray<BLESyncSession>(success)
					onCharacteristicsChanged?.invoke(device, characteristic, bytes)
				}
				else -> {}
			}
