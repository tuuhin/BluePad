...
extern "C" {
JNIEXPORT jlong JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeCreate(JNIEnv* env,
                                                                                 jobject obj) {
    init_apartment();
...
JNIEXPORT void JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeDestroy(JNIEnv* env,
                                                                                 jobject,
                                                                                 jlong id) {
    ...
    try {
        if (ctx->service_provider) {
            ...
            WIN_LOG(L"ADVERTISEMENT STATUS :"
...
JNIEXPORT void JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeRegisterCallback(
    JNIEnv* env, jobject, const jlong h, jobject cb) {
...
JNIEXPORT jint JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeGetAdvertisingStatus(
    JNIEnv*, jobject, const jlong h) {
...
Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeStopAdvertising(JNIEnv*, jobject, const jlong h) {
    const auto* ctx = reinterpret_cast<NativeContext*>(h);
    if (!ctx) {
        WIN_LOG("INVALID CONTEXT PROVIDED");
        return;
    ...
    try {
        ...
        auto current_status = ctx->service_provider.AdvertisementStatus();
        WIN_LOG(L"CURRENT ADVERTISEMENT STATUS (Status :"
        ...
        if (current_status == GattServiceProviderAdvertisementStatus::Started ||
            ...
            auto updated_status = ctx->service_provider.AdvertisementStatus();
            WIN_LOG(L"CURRENT ADVERTISEMENT STATUS (Status :"
                    << std::to_wstring(static_cast<int32_t>(updated_status)) << L")");
        }
    } catch (hresult_error const& ex) {
        WIN_LOG(L"WinRT Exception: " << ex.message().c_str());
    } catch (...) {
...
JNIEXPORT void JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeStartAdvertising(
    JNIEnv* env, jobject, const jlong h, jobject config) {
    const auto* ctx = reinterpret_cast<NativeContext*>(h);
    if (!ctx) {
        WIN_LOG("INVALID CONTEXT PROVIDED");
        return;
    ...
    auto serviceDataArray =
        reinterpret_cast<jbyteArray>(env->GetObjectField(config, serviceDataId));
    ...
    if (serviceDataArray != nullptr) {
        ...
        writer.WriteBytes(winrt::array_view<uint8_t const>(
            reinterpret_cast<uint8_t*>(buffer), reinterpret_cast<uint8_t*>(buffer) + len));
...
JNIEXPORT void JNICALL Java_com_sam_blejavaadvertise_BLEAdvertiser_nativeAddService(
    JNIEnv* env, jobject, jlong h, jobject jService) {
    auto* ctx = reinterpret_cast<NativeContext*>(h);
    if (!ctx) {
        WIN_LOG("INVALID CONTEXT PROVIDED");
        return;
    ...
    env->ReleaseStringUTFChars(jSvcUuid, svcUuidStr);
    ...
    env->CallVoidMethod(ctx->callback, onServiceAdded, jSvcUuid,
    ...
    for (int i = 0; i < count; i++) {
        ...
        jmethodID characteristics_property_write =
            env->GetMethodID(charCls, "canWriteRequest", "()Z");
        jmethodID characteristics_property_write_no_response =
            env->GetMethodID(charCls, "canWriteCommand", "()Z");
        ...
        bool can_write_without_response =
            env->CallBooleanMethod(jChar, characteristics_property_write_no_response);

        const char* charUuidStr = env->GetStringUTFChars(jCharUuid, nullptr);
        std::wstring charUuidW  = to_hstring(charUuidStr).c_str();
        env->ReleaseStringUTFChars(jCharUuid, charUuidStr);
        ...
        auto charResult = ctx->service_provider.Service()
                              .CreateCharacteristicAsync(guid(charUuidW), params)
                              .get();

        auto characteristic = charResult.Characteristic();
        ctx->characteristics.insert(pair(charUuidW, characteristic));
        ...
        characteristic.ReadRequested([ctx, charUuidW,
                                      serviceUuidW](GattLocalCharacteristic const& characteristic,
            ...
            if (ctx->vm_ref->GetEnv(reinterpret_cast<void**>(&jni_env), JNI_VERSION_1_6) !=
                JNI_OK) {
            ...
            jmethodID read_characteristics_method =
                jni_env->GetMethodID(callback_class, "onReadCharacteristics",
            ...
            jstring service_uuid_str =
                jni_env->NewStringUTF(winrt::to_string(serviceUuidW).c_str());
            jstring characteristics_uuid_str =
                jni_env->NewStringUTF(winrt::to_string(charUuidW).c_str());

            const auto read_response = reinterpret_cast<jbyteArray>(jni_env->CallObjectMethod(
                ctx->callback, read_characteristics_method, external_device_address,
            ...
            if (read_response == nullptr) {
                ...
                WIN_LOG(L"CHARACTERISTIC " << to_hstring(characteristic.Uuid())
                                           << "ATTRIBUTE NOT FOUND");
            } else {
                ...
                WIN_LOG(L"READ CHARACTERISTIC " << to_hstring(characteristic.Uuid())
                                                << "DATA SEND SIZE "
        ...
        characteristic.WriteRequested([ctx, charUuidW,
                                       serviceUuidW](GattLocalCharacteristic const& characteristic,
            ...
            jstring characteristics_uuid_str =
                jni_env->NewStringUTF(winrt::to_string(charUuidW).c_str());
            jbyteArray write_request_value =
                jni_env->NewByteArray(static_cast<jsize>(buffer.Length()));
            jni_env->SetByteArrayRegion(write_request_value, 0, static_cast<jsize>(buffer.Length()),
                                        reinterpret_cast<const jbyte*>(buffer.data()));

            jni_env->CallObjectMethod(ctx->callback, on_write_characteristics_method,
                                      external_device_address, service_uuid_str,
    ...
    ctx->service_provider.AdvertisementStatusChanged(
        [ctx](GattServiceProvider const&,
            ...
            jni_env->CallVoidMethod(ctx->callback, device_status_change_method,
