...
constexpr uint16_t DISCOVERY_SERVICE_ID = 0xFEE7; // U1
constexpr uint16_t DATA_SERVICE_ID      = 0xFEE8; // U2
...
IBuffer StringToIBuffer(const std::string& text) {
    ...
    writer.WriteBytes(
        array_view<const uint8_t>(reinterpret_cast<const uint8_t*>(text.data()),
...
int ble_advertisement() {
    ...
    try {
        ...
        characteristic.ReadRequested(
            [](GattLocalCharacteristic const& sender, GattReadRequestedEventArgs const& args) {
